...
public class Framework{
	  ...
	  public int tempo_test(Song s) {  //rules to test tempo < 200 we can modify the number
		assertTrue(s.gettempo()<200);
		return 1;  
	  }	
	  
	  public int instrument_test(Song s) { //rules to test instrument * in allowed range
	  ...
	  public int chordmap_test(Song s) { //rules to test chordmap *there can be no chords as well
	  ...
	  public int rhythm_test(Song s) { //rules to test rhythm *there can be no rhythm as well
		  //rhythm allowed
			  ...
			  for(int ii=0;ii<p.size();ii++) {
				  ...
				  for(int j=0;j<notes0.size();j++) {
					  ...
					  for(int o=0;o<arr.length;o++) {
						  if(ar.get(o).contains("i")) {
							  num = num+1;
							  assertTrue(ar.contains("i"));
							  continue;
						  }
						  if(ar.get(o).contains("q")) {
							  num = num+1;
							  assertTrue(ar.contains("q"));
							  continue;
						  }
						  if(ar.get(o).contains("h")) {
							  num = num+1;
